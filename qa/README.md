...
```
mvn clean install -Pengine-integration-jakarta,tomcat,h2
```
...
* Domain mode: `mvn clean install -Pengine-integration-jakarta,h2,wildfly-domain`
...
and `${integration-test-id}` can be either `engine-integration-jakarta` or `webapps-integration`.
