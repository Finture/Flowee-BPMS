...
public abstract class ReferenceWalker<T> {
  ...
  protected List<com.finture.bpm.engine.impl.tree.TreeVisitor<T>> preVisitor = new ArrayList<com.finture.bpm.engine.impl.tree.TreeVisitor<T>>();

  protected List<com.finture.bpm.engine.impl.tree.TreeVisitor<T>> postVisitor = new ArrayList<com.finture.bpm.engine.impl.tree.TreeVisitor<T>>();
  ...
  public ReferenceWalker<T> addPreVisitor(com.finture.bpm.engine.impl.tree.TreeVisitor<T> collector) {
    this.preVisitor.add(collector);
    return this;
  }

  public ReferenceWalker<T> addPostVisitor(com.finture.bpm.engine.impl.tree.TreeVisitor<T> collector) {
  ...
  public T walkWhile(WalkCondition<T> condition) {
    while (!condition.isFulfilled(getCurrentElement())) {
      for (com.finture.bpm.engine.impl.tree.TreeVisitor<T> collector : preVisitor) {
      ...
      for (com.finture.bpm.engine.impl.tree.TreeVisitor<T> collector : postVisitor) {
        collector.visit(getCurrentElement());
      }
    }
