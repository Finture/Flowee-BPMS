 ...
 */
package com.finture.bpm.engine.impl.jobexecutor;
...
import com.finture.bpm.engine.impl.cmd.ExecuteJobsCmd;
import com.finture.bpm.engine.impl.interceptor.Command;
import com.finture.bpm.engine.impl.interceptor.CommandExecutor;
import com.finture.bpm.engine.impl.interceptor.ProcessDataContext;
import com.finture.bpm.engine.impl.jobexecutor.FailedJobListener;
import com.finture.bpm.engine.impl.jobexecutor.JobExecutorLogger;
import com.finture.bpm.engine.impl.jobexecutor.JobFailureCollector;
import com.finture.bpm.engine.impl.jobexecutor.SuccessfulJobListener;
...
public class ExecuteJobHelper {
  ...
  public static void executeJob(String jobId, CommandExecutor commandExecutor) {
    ...
    com.finture.bpm.engine.impl.jobexecutor.JobFailureCollector jobFailureCollector = new com.finture.bpm.engine.impl.jobexecutor.JobFailureCollector(jobId);

    executeJob(jobId, commandExecutor, jobFailureCollector, new ExecuteJobsCmd(jobId, jobFailureCollector));

  }

  public static void executeJob(String nextJobId, CommandExecutor commandExecutor, com.finture.bpm.engine.impl.jobexecutor.JobFailureCollector jobFailureCollector, Command<Void> cmd) {
  ...
  protected static void invokeJobListener(CommandExecutor commandExecutor, com.finture.bpm.engine.impl.jobexecutor.JobFailureCollector jobFailureCollector) {
    if(jobFailureCollector.getJobId() != null) {
      if (jobFailureCollector.getFailure() != null) {
        ...
        com.finture.bpm.engine.impl.jobexecutor.FailedJobListener failedJobListener = createFailedJobListener(commandExecutor, jobFailureCollector);
        ...
        com.finture.bpm.engine.impl.jobexecutor.SuccessfulJobListener successListener = createSuccessfulJobListener(commandExecutor);
   ...
   */
  private static OptimisticLockingException callFailedJobListenerWithRetries(CommandExecutor commandExecutor, com.finture.bpm.engine.impl.jobexecutor.FailedJobListener failedJobListener) {
    try {
  ...
  protected static void handleJobFailure(final String nextJobId, final com.finture.bpm.engine.impl.jobexecutor.JobFailureCollector jobFailureCollector, Throwable exception) {
    jobFailureCollector.setFailure(exception);
  }


  protected static com.finture.bpm.engine.impl.jobexecutor.FailedJobListener createFailedJobListener(CommandExecutor commandExecutor, JobFailureCollector jobFailureCollector) {
    return new FailedJobListener(commandExecutor, jobFailureCollector);
  }

  protected static com.finture.bpm.engine.impl.jobexecutor.SuccessfulJobListener createSuccessfulJobListener(CommandExecutor commandExecutor) {
