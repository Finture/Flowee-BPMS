...
// mvn central
image::https://maven-badges.herokuapp.com/maven-central/com.finture.bpm.springboot/camunda-bpm-spring-boot-starter/badge.svg[link="https://maven-badges.herokuapp.com/maven-central/com.finture.bpm.springboot/camunda-bpm-spring-boot-starter"]
// openhub
...
This project provides Spring Boot starters that allow you to integrate the Flowee BPMS into your Spring Boot applications.

## Supported Spring line

The Flowee BPMS Spring Boot starter supports Spring Boot 4 / Spring Framework 7. Removed legacy Spring starter/runtime integration is excluded from release validation.
...
* link:SPRING_BOOT_4_MIGRATION_PLAN.md[Flowee-BPMS Spring Boot 4 migration plan]
...
[IMPORTANT]
We have decided to refactor the configuration mechanism. Especially, we removed the  `com.finture.bpm.spring.boot.starter.configuration.CamundaConfiguration` interface. For now, each configuration is a `com.finture.bpm.engine.impl.cfg.ProcessEnginePlugin`. With this it is possible to hook into `preInit`, `postInit` and `postProcessEngineBuild`, which supports more complex situations.
If you have implemented a `CamundaConfiguration`, you have to switch to `ProcessEnginePlugin` (and its `preInit` method). You can also extend your configuration from `com.finture.bpm.spring.boot.starter.configuration.impl.AbstractCamundaConfiguration` to avoid a cast to `SpringProcessEngineConfiguration`.
...
Starting from version 2.3.0 it was adopted as part of the Flowee BPMS.
