 ...
 */
package com.finture.bpm.engine.rest.util.container;
...
import com.finture.bpm.engine.rest.CustomJacksonDateFormatTest;
import com.finture.bpm.engine.rest.ExceptionHandlerTest;
import com.finture.bpm.engine.rest.application.TestCustomResourceApplication;
import com.finture.bpm.engine.rest.security.auth.ProcessEngineAuthenticationFilter;
import com.finture.bpm.engine.rest.standalone.NoServletAuthenticationFilterTest;
import com.finture.bpm.engine.rest.standalone.NoServletEmptyBodyFilterTest;
import com.finture.bpm.engine.rest.standalone.ServletAuthenticationFilterTest;
import com.finture.bpm.engine.rest.standalone.ServletEmptyBodyFilterTest;
  ...
  static {
        ...
        Servlets.deployment()
            ...
            .addFilter(Servlets.filter("camunda-auth", ProcessEngineAuthenticationFilter.class)
                .addInitParam("authentication-provider",
                    "com.finture.bpm.engine.rest.security.auth.impl.HttpBasicAuthenticationProvider"))
            ...
            .addServlet(Servlets.servlet("camunda-app", HttpServletDispatcher.class)
                ...
                .addInitParam("jakarta.ws.rs.Application",
                    "com.finture.bpm.engine.rest.util.container.JaxrsApplication"))));
    ...
    TEST_RULE_FACTORIES.put(ServletEmptyBodyFilterTest.class, new UndertowServletContainerRuleFactory(
        Servlets.deployment()
            ...
            .setClassLoader(ResteasyUndertowServerBootstrap.class.getClassLoader())
            .addListener(Servlets.listener(ResteasyBootstrap.class))
            .addFilter(Servlets.filter("EmptyBodyFilter", com.finture.bpm.engine.rest.filter.EmptyBodyFilter.class)
                .addInitParam("rest-url-pattern-prefix", ""))
    ...
    TEST_RULE_FACTORIES.put(CustomJacksonDateFormatTest.class, new UndertowServletContainerRuleFactory(
        Servlets.deployment()
            ...
            .setClassLoader(ResteasyUndertowServerBootstrap.class.getClassLoader())
            .addListener(Servlets.listener(ResteasyBootstrap.class))
            .addListener(Servlets.listener(com.finture.bpm.engine.rest.CustomJacksonDateFormatListener.class))
            .addInitParameter("com.finture.bpm.engine.rest.jackson.dateFormat", "yyyy-MM-dd'T'HH:mm:ss")
            .addFilter(Servlets.filter("Resteasy", FilterDispatcher.class)
                .addInitParam("jakarta.ws.rs.Application",
                    "com.finture.bpm.engine.rest.util.container.JaxrsApplication"))
            .addFilterUrlMapping("Resteasy", "/*", DispatcherType.REQUEST)));
