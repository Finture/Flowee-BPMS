...
import com.finture.bpm.spring.boot.starter.rest.CamundaBpmRestInitializer;
import com.finture.bpm.spring.boot.starter.rest.CamundaJerseyResourceConfig;
...
public class FloweeBPMSBpmRunRestConfiguration {

  @Autowired
  FloweeBPMSBpmRunProperties floweeBPMSBpmRunProperties;
  ...
  public FilterRegistrationBean<Filter> processEngineAuthenticationFilter(JerseyApplicationPath applicationPath) {
    ...
    FloweeBPMSBpmRunAuthenticationProperties properties = floweeBPMSBpmRunProperties.getAuth();
    if (properties.getAuthentication() == null || FloweeBPMSBpmRunAuthenticationProperties.DEFAULT_AUTH.equals(properties.getAuthentication())) {
      registration.addInitParameter("authentication-provider", "com.finture.bpm.engine.rest.security.auth.impl.HttpBasicAuthenticationProvider");
    }
    return registration;
  }
  ...
  public CamundaJerseyResourceConfig camundaRunJerseyResourceConfig() {
    ...
    camundaJerseyResourceConfig.setProperties(Collections.singletonMap("jersey.config.server.wadl.disableWadl", floweeBPMSBpmRunProperties.getRest().isDisableWadl()));
