 ...
 */
package com.finture.bpm.run.test.plugins;
...
import com.finture.bpm.run.property.FloweeBPMSBpmRunProcessEnginePluginProperty;
import com.finture.bpm.run.utils.FloweeBPMSBpmRunProcessEnginePluginHelper;
import org.junit.Test;

public class FloweeBPMSBpmRunProcessEnginePluginsHelperTest {

  @Test
  public void shouldReportMissingPluginClass() {
    // given
    // a process engine plugins map with a class not on the classpath
    FloweeBPMSBpmRunProcessEnginePluginProperty pluginConfig = new FloweeBPMSBpmRunProcessEnginePluginProperty();
    pluginConfig.setPluginClass("com.finture.bpm.run.test.plugins.TestThirdPlugin");
    pluginConfig.setPluginParameters(Collections.EMPTY_MAP);
    // a process engine plugins map with a plugin not configured properly
    List<FloweeBPMSBpmRunProcessEnginePluginProperty> plugins =
        Collections.singletonList(pluginConfig);
    List<ProcessEnginePlugin> pluginList = Collections.EMPTY_LIST;

    // when
    assertThatThrownBy(() -> FloweeBPMSBpmRunProcessEnginePluginHelper.registerYamlPlugins(pluginList,
                                                                                        plugins))
        // then
        ...
        .hasMessageContaining("Unable to register the process engine plugin " +
                                  "'com.finture.bpm.run.test.plugins.TestThirdPlugin'.");
  }
  ...
  public void shouldReportWrongPluginClass() {
    ...
    FloweeBPMSBpmRunProcessEnginePluginProperty pluginConfig = new FloweeBPMSBpmRunProcessEnginePluginProperty();
    pluginConfig.setPluginClass("com.finture.bpm.run.test.plugins.TestFalsePlugin");
    pluginConfig.setPluginParameters(Collections.EMPTY_MAP);
    // a process engine plugins map with a plugin not configured properly
    List<FloweeBPMSBpmRunProcessEnginePluginProperty> plugins =
        Collections.singletonList(pluginConfig);
    ...
    assertThatThrownBy(() -> FloweeBPMSBpmRunProcessEnginePluginHelper.registerYamlPlugins(pluginList,
        ...
        // an exception is thrown with a user-friendly message asking to check the plugin class
        .isInstanceOf(ProcessEngineException.class)
        .hasMessageContaining("'com.finture.bpm.run.test.plugins.TestFalsePlugin'. " +
  ...
  public void shouldReportMissingPluginConfigurationProperty() {
    // given
    FloweeBPMSBpmRunProcessEnginePluginProperty pluginConfig = new FloweeBPMSBpmRunProcessEnginePluginProperty();
    pluginConfig.setPluginClass("com.finture.bpm.run.test.plugins.TestFirstPlugin");
    ...
    assertThatThrownBy(() -> FloweeBPMSBpmRunProcessEnginePluginHelper.registerYamlPlugins(pluginList,
        ...
        .hasMessageContaining("Please check the configuration options for plugin " +
                                  "'com.finture.bpm.run.test.plugins.TestFirstPlugin'.");
  }
