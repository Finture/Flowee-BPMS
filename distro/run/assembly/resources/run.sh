...
OPTIONS_HELP="Options:
  --webapps    - Enables the Flowee BPMS Webapps
  --oauth2     - Enables the Flowee BPMS Spring Security OAuth2 integration
...
Please stop it or remove the file $PID_PATH."
    ...
    "$JAVA" -Dloader.path="$classPath" -Dflowee-bpms.deploymentDir="$DEPLOYMENT_DIR" $JAVA_OPTS -jar "$BASEDIR/flowee-bpms-bpm-run-core.jar" --spring.config.location=file:"$configuration" &
