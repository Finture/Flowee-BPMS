...
FOR /f "delims=. tokens=1" %%v in ("%JAVA_VERSION%") do (
  IF %%v LSS %EXPECTED_JAVA_VERSION% (
    ECHO You must use at least JDK 17 to start Flowee BPMS Run.
    GOTO :EOF
  )
)
...
IF [%detachProcess%]==[true] (
  ...
  start "%APPNAME%" "%JAVA%" -Dloader.path="%classPath%" -Dflowee-bpms.deploymentDir="%DEPLOYMENTDIR%" %JAVA_OPTS% -jar "%BASEDIR%flowee-bpms-bpm-run-core.jar" --spring.config.location=file:"%configuration%"

) ELSE (
  call "%JAVA%" -Dloader.path="%classPath%" -Dflowee-bpms.deploymentDir="%DEPLOYMENTDIR%" %JAVA_OPTS% -jar "%BASEDIR%flowee-bpms-bpm-run-core.jar" --spring.config.location=file:"%configuration%"
)
...
:ArgsHelp
ECHO Options:
ECHO   --webapps    - Enables the Flowee BPMS Webapps
ECHO   --oauth2     - Enables the Flowee BPMS Spring Security OAuth2 integration
